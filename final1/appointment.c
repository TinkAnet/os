...
date_t holidays[] = {
    {20230305, 2023, 3, 5}, {20230312, 2023, 3, 12}, {20230319, 2023, 3, 19}, {20230326, 2023, 3, 26}, {20230329, 2023, 3, 29}, {20230330, 2023, 3, 30},
    {20230501, 2023, 5, 1}, {20230507, 2023, 5, 7}, {20230514, 2023, 5, 14}, {20230521, 2023, 5, 21}, {20230526, 2023, 5, 26}, {20230528, 2023, 5, 28}, 
...
schd_t load_schd(int id, int caller, int num_of_callee, int *callee, int type, long long start_time, double len){
    ...
    if(ret.start_slot < 0 || ret.end_slot < 0)
        ret.type = -1;
    if((ret.start_slot-1) / SLOT_PER_DAY != (ret.end_slot-1) / SLOT_PER_DAY)
        ret.type = -1;
    ret.priv = 0;
    ret.if_user_choose_to_reject = 0;
    return ret;
}
/**
 * @brief 
